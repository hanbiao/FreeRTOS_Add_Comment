...
#define portSTACK_GROWTH            ( -1 )  //栈采用从高地址到低地址的方式, portSTACK_GROWTH < 0, 同时也表示栈递增的时候地址要减1
...
/* Architecture specific optimizations. */
    ...
    #define configUSE_PORT_OPTIMISED_TASK_SELECTION 1   //如果优先级小于32, 可以通过位操作来优化任务选择, 可以提高查找效率
#else
    ...
    static portFORCE_INLINE uint32_t ucPortCountLeadingZeros( const uint32_t ulBitmap )
    {
        ...
        __asm__ volatile ("cntlzw %0, %1" : "=r" (ucReturn) : "r" (ulBitmap)); //找一个32bit变量中,从高位算起到低位, 有多少个连续的位为0
    ...
    #define portGET_HIGHEST_PRIORITY( uxTopPriority, uxReadyPriorities ) uxTopPriority = ( 31UL - ucPortCountLeadingZeros( ( uxReadyPriorities ) ) )  //直到位值为1的最高位, 也就是最高优先级
